...
tab_presc_sum <- function(dt, var, group){
  # Tabulate the total and relative number of prescriptions
  ...
  dt <- dt[, .(value = `n_%`(sum(num_abx), unique(n_group))), by = c(group, var)]
  setnames(dt, var, "var")
  dt
}

abx_denom <- substitute(sum(num_abx))
...
base_presc <- invoke_map(tbl_def(tab_presc_sum), .x = list(list(dt = baseline, show_miss = FALSE,
                                                                fun.group = abx_denom, aux = "num_abx")))
...
tbl_base <- 
  map2(base_counts, base_presc, merge, by = "var", all.x = TRUE, sort = FALSE) %>% 
  map2(base_rates, merge, by = "var", all.x = TRUE, sort = FALSE) %T>%
...
tbl_base %>% 
  kable(
    ...
    align = c("l", rep("r", 3)),
    escape = FALSE,
    col.names = c("Patient characteristics", "n (\\%)", "n (\\%)", "Crude Rate (95\\%-CI)", "Unadj. RR (95\\%-CI)"),
    caption = if(format == "html") "Baseline table" else "(ref:cap-tab-baseline)"
  ) %>% 
  kable_styling() %>% 
  add_header_above(c(" " = 2, "Number of prescriptions" = 3))%>% 
...
tbl_cross <- function(var1, var2, method = "rate", with_ci = TRUE){
  ...
  if(method == "rate"){
    ...
    res <- pmap_df(combs,
                ~ data.table(var1 = ..1, 
                             var2 = ..2,
                             est = sum(baseline[get(var1) == ..1 & get(var2) == ..2, num_abx]) / sum(baseline$num_abx)))
  }
...
```{r fig-abx-rate, fig.cap = "Average annual rate of first line, second line and specific antibiotic therapies by the number of AECOPD per year. A. First line antibiotic therapy, B. Second line antibiotic therapy, C. Azithromycin as a marker of phrophylactic antibiotic use in COPD, D. Nitrofurantoin as a marker of prescribing for urinary tract infection, E. Flucloxacillin as a marker for prescribing for skin and soft tissue infections", fig.width=5.5, fig.height=7.1, dpi=900}
...
abx_rates <- copy(abx_rates$rate[grp != "all"]) # copy to get rid of "shallow-copy" data.table message
...
levels(abx_rates$grp) <- c("0", "1 P", "2 P", "\u22653 P", "\u22651 H")

g_other <- g %+% 
  (abx_rates[!var %in% c("first line", "other")] %>% 
...
g_comb <- ggpubr::ggarrange(g_first, g_other, nrow = 2, ncol = 1, heights = c(2, 3))
ggpubr::annotate_figure(g_comb, left = ggpubr::text_grob("Rate per year", rot = 90))
...
```{r fig-ind, fig.cap="Indication for the antibiotic prescription, grouped by the number of AECOPD during follow-up. Excludes prescriptions with missing indication.", fig.width=6.5, fig.height=3, dpi=900}
...
fig_ind_data[, fu_ae_cat := factor(as.character(fu_ae_cat), c("0", "1", "2", "3+", "1+ severe"), 
                               c("0", "1 PC", "2 PC", "\u22653 PC", "\u22651 HOSP"))]
...
```{r fig-ind-miss, fig.cap="Proportion of antibiotic prescriptions that are missing a recorded indication, grouped by the number of AECOPD during follow-up", fig.width=6.5, fig.height=3, dpi=900}
...
g_mosaic <- 
  ggplot(NULL, aes(xmin = x_start, xmax = x_end,
    ...
    geom_text(aes(x = (x_start + x_end) / 2, y = (y_start + y_end) / 2, 
                  ...
                  size = pmax(3, pmin(6, perc_abx * 100))), colour = "white") + 
...
```{r fig-mosaic-sev-fu, warning = FALSE, fig.cap="Relationship between disease severity (assessed by FEV1), rate of antibiotic prescribing according to the number of AECOPD and total antibiotic use. Tile sizes are scaled to reflect the proportion of antibiotics that were prescribed to patients in each group. For example, patients with mild FEV and zero AECOPD during follow up were prescribed an average of 1.28 (1.20-1.37) antibiotics per year and accounted for 5.1% of the total antibiotics prescribed to patients with COPD.", fig.width = 6.5, fig.height = 6.5, dpi=900}
...
```{r fig-mosaic-ae-fu, warning = FALSE, message= FALSE, fig.cap="Relationship between disease severity (assessed by FEV1), rate of antibiotic prescribing according to the number of AECOPD and total antibiotic use. Tile sizes are scaled to reflect the proportion of antibiotics that were prescribed to patients in each group. For example, patients with zero AECOPD during baseline and with zero AECOPD during follow up were prescribed an average of 1.22 (1.18-1.27) antibiotics per year and accounted for X% of the total antibiotics prescribed to patients with COPD.", fig.width = 6.5, fig.height = 6.5, dpi=900}

# PUBLICATION SUPPLEMENT: Figure 3
...
multivar <- function(dt, severe_var){
  # Run the multivariate analysis of severity measures, adjusting for 
  # all other baseline variables and practice level random effects
  #
  # Args:
  #   dt - data.table containing all baseline measures
  #   severe_var - name of the severity measure
  #
  # Result:
  #   a glmmTMB model containing the fit
  
  form <- as.formula(str_c("num_abx ~ age_cat_ctr * female + imd_ctr + 
                              asthma + chd + ckd + dm + hf + pad + stroke + obese + smoke + 
                              vacc + ", severe_var, " + (1 | pracid)"))
  
  glmmTMB(form, data = dt, family = nbinom2, offset = log(risk))
}

multi_coefs <- function(mod){
  coefs <- tidy(mod)
  setDT(coefs)
  coefs %<>% .[component == "cond"]
  ...
  coefs[, .(term, rr, ci = str_c("(", lower, "-", upper, ")"), p.value)]
}
...
full_fev <- multivar(baseline, "fev_ctr")

multi_coefs(full_fev) %>% 
  kable(
    format, 
    booktabs = TRUE,
    linesep = "",
...
full_mrc <- multivar(baseline, "mrc_ctr")

multi_coefs(full_mrc) %>% 
  kable(
    format, 
    booktabs = TRUE,
    linesep = "",
...
full_ae <- multivar(baseline, "aecopd")
...
multi_coefs(full_ae) %>% 
  kable(
    ...
    escape = FALSE,
    col.names = c("Coefficient", "RR", "(95%-CI)", "p-value"),
    caption = "Multivariate analysis of AECOPD during baseline"
  )
